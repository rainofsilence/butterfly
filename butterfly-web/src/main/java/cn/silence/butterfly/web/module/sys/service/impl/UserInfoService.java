...
import cn.silence.butterfly.web.module.sys.common.IUserCheck;
import cn.silence.butterfly.web.module.sys.constant.EnumConstant;
    ...
    public BaseResponse<PageResult<UserVO>> page(UserPageRequest pageRequest) {
        ...
        List<UserVO> dataList = BeanPlusUtils.copyProperties(userInfoList, UserVO.class).stream().peek(d -> {
            if (StrUtils.isNotBlank(d.getGender())) {
                d.setGenderName(iEnumRedisService.getItemName(EnumConstant.GENDER, d.getGender()));
            }
